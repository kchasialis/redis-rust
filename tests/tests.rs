...
use redis_rs::{
    handle_blpop_cmd, handle_echo_cmd, handle_get_cmd, handle_llen_cmd, handle_lpop_cmd,
    handle_lpush_cmd, handle_lrange_cmd, handle_ping_cmd, handle_rpush_cmd, handle_set_cmd,
...
mod set_get {
    ...
    async fn basic_set_then_get() {
        let s = make_storage();
        assert_eq!(
            handle_set_cmd(&cmd(&["SET", "orange", "blueberry"]), s.clone()).await,
    ...
    async fn set_overwrites_existing_key() {
        let s = make_storage();
        handle_set_cmd(&cmd(&["SET", "k", "v1"]), s.clone()).await;
        handle_set_cmd(&cmd(&["SET", "k", "v2"]), s.clone()).await;
        assert_eq!(
    ...
    async fn px_expiry() {
        let s = make_storage();
        handle_set_cmd(&cmd(&["SET", "apple", "strawberry", "PX", "100"]), s.clone()).await;
    ...
    async fn ex_expiry_alive_within_window() {
        let s = make_storage();
        handle_set_cmd(&cmd(&["SET", "key", "value", "EX", "10"]), s.clone()).await;
        assert_eq!(
...
mod type_cmd {
    ...
    async fn string_type() {
        let s = make_storage();
        handle_set_cmd(&cmd(&["SET", "blueberry", "raspberry"]), s.clone()).await;
        assert_eq!(
...
mod llen {
    ...
    async fn existing_and_missing() {
        ...
        assert_eq!(
            handle_llen_cmd(&cmd(&["LLEN", "orange"]), s.clone()).await,
            RespValue::Integer(5)
        );
        assert_eq!(
            handle_llen_cmd(&cmd(&["LLEN", "missing_key"]), s).await,
            RespValue::Integer(0)
        );
    }
}
